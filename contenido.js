...
(function () {
  ...
  var TEXTO_MATRIX = [
    ...
    "- MATRIX abre con una pantalla negra que pide un PIN de máximo seis dígitos. No hay usuario ni contraseña, no existe una cuenta suya: todos los que abren ese computador entran con el mismo PIN. Si alguien dejó el PIN vacío en Ajustes, la aplicación abre directo sin preguntar nada.",
    "- Si se equivoca, sale un texto rojo que dice PIN incorrecto, se borra la casilla y puede volver a intentar. No hay bloqueo por intentos y no queda registro de quién entró.",
    ...
    "- Para volver a modo gerencia se hace clic en el sello M rojo de la esquina de arriba a la izquierda y se escribe el PIN. Y aquí le digo la verdad completa, porque el curso no le va a mentir: ese modo solo esconde los botones. Los datos de todo el equipo siguen guardados en ese mismo computador, así que no dé por hecho que sus números son privados.",
    ...
    "- En Registro cada celda que se escribe cambia lo que alguien cobra, al instante. No hay botón de guardar: guarda al salir de la celda, y ahí mismo se recalculan Tablero y Comisiones. No hay deshacer y no queda rastro de quién cambió qué.",
    "- Además no hay permisos: cualquiera que entre con el PIN puede editar las cifras de cualquiera. Si usted ve un número raro en Registro, no lo corrija: avísele a su supervisor.",
    "- Esa tabla tiene una equis al final de cada fila que borra la línea entera con sus ocho números, y esa no pregunta nada. Casi todo lo demás en MATRIX pide confirmación; esta no.",
    "- Las casillas de números borran letras, comas y puntos. Si alguien escribe dos coma cinco, queda veinticinco. Ahí no existen los decimales.",
    "- El botón Pegar tabla carga a mucha gente de golpe, y tiene dos trampas. Si el nombre pegado no coincide exactamente con uno que ya existe, crea un asesor nuevo: Ana Gómez con tilde y Ana Gomez sin tilde se vuelven dos personas, la cartera queda partida en dos y ninguna llega al escalón.",
    "- Por eso, después de pegar, lo primero que hay que leer es el mensaje final: dice cuántos asesores nuevos creó. Si dice tres nuevos y no contrataron a nadie, hay tres nombres mal escritos.",
    "- La segunda trampa del pegado: si no se marca la casilla de acumular varios días, lo pegado reemplaza lo que había en vez de sumarse, y se pierden los días acumulados.",
    "- El botón Traer del ADMIN crea un periodo nuevo cada vez que se pulsa, sin avisar si ya existe uno igual. Dos clics y quedan dos quincenas idénticas en la lista, y es fácil pagar mirando la equivocada.",
    "- La pantalla Comisiones no se toca. Ahí vive la tabla de escalones que decide el pago de todos los de esa categoría, la equis de cada escalón no pregunta nada, y hay un botón que restaura las tablas de fábrica y reemplaza de un golpe cualquier ajuste que la empresa hubiera hecho.",
    "- Para sacar a alguien del equipo nunca se usa Eliminar: eso borra su captura de resultados en todos los periodos, incluidos los ya pagados. Lo correcto es ponerle fecha de salida, que lo deja inactivo y conserva la historia. Y ojo, poner fecha de salida apaga a la persona aunque el estado siga diciendo Activo.",
    "- En Ajustes hay un botón de Importar respaldo que reemplaza toda la base de un archivo sin preguntar absolutamente nada. Y al lado, Borrar todo. Como no hay servidor, lo que se pierde ahí no está en ninguna otra parte.",
    "- El respaldo en JSON se descarga con todo adentro, incluido el PIN, la clave de la inteligencia artificial y los sueldos de todo el mundo, en texto legible. Trátelo como trataría una nómina impresa: no se manda por WhatsApp ni se deja en la carpeta de descargas.",
    "- Y una última: la caja que dice Pregúntale a MATRIX manda a un servicio externo un resumen de la operación real, con nombres, cuentas, recuperado y comisiones, y cada pregunta gasta la cuenta del dueño de la clave. No es un juguete para probar.",
    "? ¿Qué hace usted si ve un número equivocado en la pantalla Registro? | no lo corrijo, no toco, avisar, supervisor, no hay deshacer, cambia comisiones | No lo corrijo yo. Esa celda cambia la comisión de alguien apenas salgo de ella, no hay deshacer y no queda rastro de quién lo cambió. Le aviso a mi supervisor.",
    "? ¿Cuál es la forma correcta de sacar a una persona que se retiró, y por qué no se usa Eliminar? | fecha de salida, inactivo, conserva, historial, eliminar, borra, periodos, pagados | Se le pone la fecha de salida: queda inactiva y se conserva su historial. Eliminar borra su captura de resultados en todos los periodos, incluidos los que ya se pagaron, y eso no se puede deshacer.",
    "? Nombre dos botones de MATRIX que un asesor nuevo no debe tocar nunca. | comisiones, tabla de pago, importar respaldo, borrar todo, eliminar, restaurar tablas, escalón | La tabla de pago de la pantalla Comisiones, porque cambia el sueldo de todos los de esa categoría, e Importar respaldo o Borrar todo en Ajustes, porque reemplazan o borran toda la base sin que haya copia en ningún servidor."
  ].join('\n');
