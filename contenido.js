...
(function () {
  ...
  var TEXTO_MATRIX = [
    ...
    "- Lo que MATRIX guarda es una línea por asesor y por categoría de mora dentro del periodo, con ocho números. Los cuatro que a usted le importan son: cuentas asignadas, cuentas cobradas, base a cobrar y dinero recuperado. Los días trabajados también van ahí adentro.",
    ...
    "- Por debajo del último escalón MATRIX no muestra cero: muestra un número en negativo, al que el sistema llama penalización. Si eso le llega a aparecer, pregúntelo antes de darlo por bueno: un descuento sobre lo que usted gana necesita autorización suya por escrito, y la ley no permite descontar por no alcanzar una meta. Lo que ve en pantalla es lo que calcula el sistema, no necesariamente lo que le pueden descontar.",
    ...
    "- Los sábados el sistema le rebaja el umbral: ese día le piden solo el ochenta por ciento de lo normal. Donde entre semana usted necesita cincuenta por ciento, el sábado necesita cuarenta.",
    ...
    "* Le quedan dos días de corte. ¿Qué le conviene más?",
    "+ Cerrar varias cuentas pequeñas para subir el porcentaje. | Le pagan por cuentas cobradas sobre asignadas, no por pesos: el escalón lo define ese porcentaje.",
    "x Perseguir las dos cuentas más grandes de la cartera. | Ese es el error de criterio más caro: dos cuentas grandes no mueven el porcentaje.",
    "x Da igual: al final se promedia todo. | El pago es escalonado, no proporcional: quedarse una décima abajo cambia el pago del día.",
    "* En Mi progreso le aparece el pago del día en rojo y en negativo. ¿Qué hace?",
    "+ Lo pregunta antes de darlo por bueno. | Es lo que calculó el sistema; un descuento sobre lo que usted gana necesita autorización suya por escrito.",
    "x Nada: es la penalización y así funciona. | No lo dé por bueno sin preguntar: no todo lo que el sistema calcula se le puede descontar.",
    "x Trabaja el doble mañana para taparlo. | Puede hacerlo, pero primero pregunte: el número en rojo puede no ser un descuento real.",
    ...
    "? ¿Qué significa que su pago por día salga en rojo y en negativo? | debajo, mínimo, tabla, preguntar, autorización, escrito | Significa que quedé por debajo del mínimo de mi tabla. En Mi progreso miro cuántas cuentas me faltan para salir de ahí, y si el número negativo me lo van a descontar lo pregunto: eso necesita autorización mía por escrito.",
    "",
    ...
    "* Antes de grabar una llamada con un cliente, ¿qué tiene que hacer?",
    "+ Avisarle y pedirle permiso. | La voz es un dato personal: grabar sin avisar expone a la empresa.",
    "x Nada: si el sistema lo permite, está autorizado. | Que el sistema lo permita no lo vuelve legal.",
    "x Avisarle solo si el cliente pregunta. | El aviso va siempre, no a petición.",
    ...
    "- Dentro del sistema hay un bloque de cumplimiento legal para Colombia. Ojo con lo que es: se escribió como guardarraíl para un módulo de mensajes que hoy está apagado, no es una política aprobada por el área legal. Se lo leo en las palabras del sistema, y después le digo qué le falta.",
    "- Y le falta lo principal: la Ley 2300 de 2023 no se nombra ni una sola vez en el sistema. Esa es la que manda sobre a qué horas se puede llamar, cuántas veces y qué hacer cuando el cliente pide que no lo contacten más. Para eso hay un curso aparte en esta misma plataforma, y ese sí es el que tiene que saberse de memoria.",
    ...
    "* El sistema lo deja llamar a las nueve de la noche. ¿Puede hacerlo?",
    "+ No: el sistema no controla la ley, y esa hora está fuera del horario permitido. | Que una herramienta no le ponga freno no significa que la ley no aplique.",
    "x Sí, porque si estuviera prohibido el sistema lo bloquearía. | MATRIX no controla horarios de contacto: la responsabilidad es del asesor.",
    "x Sí, si el cliente contesta y no se queja. | La infracción existe aunque el cliente no reclame.",
    "",
